#!/usr/bin/env bash
# power-statement-assistant - Windows WSL Setup Script
# Optimized for minimal vertical space with running timer
#
# REFERENCE IMPLEMENTATION:
#     https://github.com/bordenet/product-requirements-assistant/blob/main/scripts/setup-windows-wsl.sh
...
Setup script for Windows WSL (Ubuntu) - Installs all dependencies for power-statement-assistant
...
PREREQUISITES:
  ...
  - Run this script from within WSL (not PowerShell)
...
if ! grep -qi microsoft /proc/version; then
    echo "Error: This script must be run in Windows WSL, not native Linux or PowerShell"
...
print_header "power-statement-assistant - Windows WSL Setup"
...
echo ""
echo "WSL Tips:"
echo "  - Access files from Windows: \\\\wsl$\\Ubuntu\\home\\<user>\\power-statement-assistant"
echo "  - Use VS Code with WSL extension for best experience"
echo ""
